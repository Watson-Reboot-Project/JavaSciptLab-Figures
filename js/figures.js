...
function Figure(editor, figureNum, outputBox, varTable) {
	var myInterpreter = null;
	var done = false;
	var intervalID;
	var funcList = [];
	var codeStr = setupFigure(figureNum);
	var runMode = false;
	var varArr = [];
	var scopeArr = [];
	var thisObj = this;
	var _runButton = document.getElementById("runButton1");
	...
	*/
	...
	function walk() {
		...
		if (myInterpreter.step() == false) {
			editor.selectLine(-1, -1);
			done = true;
		}
		else {
			res = editor.selectLine(start, end, varArr.length);
			test();
		}
	...
	function updateVariables(mode, scope, leftValue, rightValue) {
		...
		if (mode == "add") {
			...
			if (!found) { varArr.push([scope, leftValue, rightValue]); scopeArr.push(scope); }
		}
		else {
			for (var i = 0; i < varArr.length; i++) {
				if (scope == varArr[i][0] && (varArr[i][1] == leftValue || varArr[i][1].data == leftValue)) {
	...
	function test() {
		for (var i = 0; i < varArr.length; i++) {
			try {
				//console.log("Testing " + varArr[i][1] + " with value: " + varArr[i][2]);
				myInterpreter.getValueFromScope(varArr[i][1]);
			}
			catch (err) {
				//console.log("Didn't find it.");
				updateVariables("del", varArr[i][0], varArr[i][1]);
			}
		}
	}
	...
	function clearTable() {
		var length = varTable.rows.length;
		for (var i = 0; i < length; i++) varTable.deleteRow[0];
	}
