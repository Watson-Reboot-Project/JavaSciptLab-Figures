...
document.write('\
	<!--\
    *************************************************************************************************\
             ************ This is string pad html **************************\
    *************************************************************************************************\
    -->\
		<div id="dialog-modal-string" title="String input Panel" style="display:none">\
		<input type="text" id="WDLStringpadInput" value=""/>\
		<input type="button" value="OK" class="OKBtnString WButton"/>\
		<input type="button" value="Clear" class="ClearBtnString WButton"/>\
		<input type="button" value="Cancel" class="CancelBtnString WButton"/>\
		</div>\
');

$(document).on('click', "input.OKBtnString" , function() 
{
	var input = $("#WDLStringpadInput");
	dialogsDefer.resolve(input.val());
	$("#dialog-modal-string").dialog("close");
});

$(document).on('click', "input.ClearBtnString" , function() 
{
	var input = $("#WDLStringpadInput").val("");
});

$(document).on('click', "input.CancelBtnString" , function() 
{
	$("#dialog-modal-string").dialog("close");
});
    		...
    		$( "#dialog-modal-hex" ).dialog(
					{
						...
						{
							if (dialogsDefer.state().localeCompare("pending")==0)
								dialogsDefer.resolve(null);
						}
					});
...
function openStringPad(title, instructions)
{
	dialogsDefer = $.Deferred();
	$("#WDLStringpadInput").val("");
	$("#dialog-modal-string").dialog({
			height: 280,
			width: 350,
			modal: true,
			close: function( event, ui )
...
function stringpadExample()
{
	openStringPad("This is a stringpad", "These are the instructions").done(function(returned)
	{
		alert(returned);	
	});
}
