 ...
 * @constructor
 */
var Interpreter = function (code, opt_initFunc, _figures) {
		...
		if (node.arguments[n]) {
			...
			if (state.func_.node) {
				...
				for (var i = 0; i < state.func_.node.params.length; i++) {
					...
					this.figures.updateVariables("add", scope, paramName, paramValue);
				}
...
Interpreter.prototype['stepVariableDeclaration'] = function () {
	...
	if (node.declarations[n]) {
		...
		this.figures.updateVariables("add", this.getScope(), node.declarations[n].id.name, "undefined");
	} else {
		this.stateStack.shift();
	}
